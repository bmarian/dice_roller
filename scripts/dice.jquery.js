class Dice{
	...
	convert_dice(){
		...
		for (var i = 0; i < this.nr_of_dices - 1; i++) {
			dice_roll = Math.floor((Math.random() * (this.type_of_dice)) + 1);
			result += dice_roll + " + ";
		}
		dice_roll = dice_roll = Math.floor((Math.random() * (this.type_of_dice)) + 1);
		result += dice_roll;
		return result;
	}
	...
	drop_lowest(number_of_drops){
		...
		if(number_of_drops < dice_rolls.length){
			for (let i = number_of_drops; i < dice_rolls.length; i++) {
				result += dice_rolls[i];
				if(i != dice_rolls.length - 1){
					result += " + ";
				}
			}
		}else{
